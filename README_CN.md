...
**AgentLens** 是一个可信 AI Agent 发现与启动平台。它帮助用户找到适合具体任务的 Agent，理解其能力和风险边界，并根据 Agent 的托管与执行方式，选择在平台内直接使用、按指南接入，或跳转到官方 / 开发者环境。
...
* 🧭 **Agent 发现与启动**：像使用轻量任务应用一样搜索、对比并打开 Agent；支持在平台内直接使用、按指南接入，或在存在执行边界时跳转到外部服务。
...
AgentLens 最新版本已完成全面重构——从纯链上 Agent 交易市场演进为**可信 AI Agent 发现、选型与启动平台**。平台把 Agent 当作面向具体任务的轻量应用：用户可以搜索自己需要的 Agent，对比结构化事实，在平台内直接使用已支持的 Agent，或在 Agent 存在明确执行边界时跳转到官方 / 开发者环境。目标是帮助用户基于证据做决策，而非依赖广告或五星好评。
...
Agent 按实际可用方式分为：**平台可用**（可在 AgentLens 内启动或租赁）、**指南可接入**（提供完整上手指南）和 **外部跳转**（因服务边界或托管归属需跳转到官方 / 开发者环境）。
