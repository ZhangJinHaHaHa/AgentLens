/**
 * 在用户采用默认排序时对已筛选目录做需求相关性重排，综合场景、标签、访问方式及中英文文本片段，返回保持类型不变的新数组。
 * 标签和场景的精确命中权重大于风险说明等低权重文本，通用型条目在缺少具体标签重叠时受罚；同分严格保留原索引顺序。
 * 该评分器无网络、状态和缓存副作用，也不会修改条目；显式选择其他排序或缺少有效需求信号时仅复制原顺序。
 * query、LLM 解析筛选和目录字段都可能来自不可信边界，分数只表示文本相关性，不得提升信任层级、绕过工作区准入或授予工具权限。
 * 输入假定已由筛选/解析层规范化；空数组、空分母和短词均安全退化，不抛错也不重试，权重变化应视为影响搜索兼容性的产品规则变更。
 */
