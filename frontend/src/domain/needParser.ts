/**
 * 需求解析的信任收敛层：从当前目录构建有限分类表，再把模型返回的原始 JSON 转换为白名单化、去重且有界的 `NeedParseResult`。
 * 解析结果进入筛选器前，还会要求原始需求明确提到审计、风险、复杂度或上手条件，防止模型凭空增加影响选择的重要约束。
 * 模块本身不调用模型、不联网、不缓存；它只处理字符串和目录快照，远端失败后的重试或本地降级由 `needParserClient` 负责。
 * 原始模型文本是不可信输入：非 JSON/非对象直接抛错，分类数组按 taxonomy 过滤，置信度夹在 0..1，未匹配词只收字符串且最多八项。
 * 缺失或类型错误的可选字段安全降级为空数组、false 或 0；输出仍只是发现提示，不能作为服务端授权、审计结论或运行准入依据。
 * taxonomy 的排序去重和显式关键词门构成可复现兼容契约，新增同义词时须避免无意放宽敏感筛选条件。
 */
import type {
  AccessType,
  AgentCatalogEntry,
