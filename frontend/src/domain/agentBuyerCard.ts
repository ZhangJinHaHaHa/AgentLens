/**
 * 将完整目录条目投影为买家卡片所需的双语摘要；显式 `buyerCard`/tagline 优先，其次使用场景，最后才落到产品类型默认文案。
 * 输入是已经汇聚的 `AgentCatalogEntry`，输出是新的展示对象；算法只读取元数据，不修改条目、不访问网络，也没有 React 状态或缓存。
 * 运行方式与数据边界根据来源、访问方式和运行时类型生成，目的是暴露托管责任，而不是证明镜像、卖家或外部 API 已通过安全验证。
 * 目录及链上合并字段都可能陈旧或由外部主体声明，界面仍应转义文本，真正的权限、隔离、计费和审计约束必须由服务端执行。
 * 缺少推荐场景、限制或运行元数据时使用确定性保守回退，不抛错也不重试；“待确认”文案不得被下游改解释为可运行承诺。
 * 双语字段和覆盖优先级构成卡片兼容契约，新增产品类型时必须同时补齐各默认映射，避免不同页面产生相互矛盾的承诺。
 */
