/**
 * 目录发现流程的纯领域内核：负责分面生成、全文匹配、多维筛选、排序、筛选芯片、零结果放宽建议及 URL 查询参数往返。
 * 每个维度内部采用“任一命中”，不同维度之间采用“全部满足”；默认排序保持目录顺序，其余排序在副本上执行，输入条目不被改写。
 * 函数不联网、不写浏览器历史或持久缓存；`URLSearchParams` 仅作为输入输出值，真正的路由状态由调用方拥有。
 * 查询参数属于浏览器不可信输入：sort/type/tier/price/auditStatus/score 会白名单收敛，布尔值只接受 `1`；其余枚举目前仅靠后续匹配失配，绝不能用于授权。
 * 合并参数时只替换本模块声明的键并保留其他查询项，这是深链和旧页面兼容不变量；CSV 会裁剪空值但不会把自由文本当可信 HTML。
 * 无匹配不会触发网络重试，放宽建议仅逐个移除当前芯片并选择结果最多者；链证据、价格和分类仍是展示快照，服务端决策必须重新验证。
 */
import type {
  AccessType,
  AgentCatalogEntry,
