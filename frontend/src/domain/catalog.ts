/**
 * 目录领域模型与汇聚规则的所有者：定义条目、能力、运行托管和工作区准入契约，并合并 marketplace、curated、listed 与链上 native 来源。
 * 合并以 token/id 为主、规范化名称为后备；编辑内容保持原样，只叠加原生链证据、定价、访问方式和标签，最终同时产出有序列表及索引。
 * 整个模块只分配对象、数组、Map/Set，不联网、不持久化也不维护跨调用缓存；源内顺序和“未匹配 native 最后追加”是稳定展示不变量。
 * 卖家声明、目录文案与 RPC 读数处于不同信任域，类型并不完成运行时校验；运行协议和产品类型推断只用于展示/路由，不能替代服务端准入、计费或隔离检查。
 * `workspaceAdmission` 是能否打开工作区的显式门，禁止再由 ID、标签或镜像提示反推；外部 URL 和自由文本应在实际使用边界继续校验与转义。
 * 缺少新字段时保留可选 marketplace 和启发式回退以兼容旧目录；重复 id 由后项覆盖，而 token 别名只在未占用时登记，数据发布阶段应消除冲突。
 * 本层没有失败重试机制，链读取失败应由上游以空 native 集合传入，使静态目录仍可用而不伪造链上证据。
 */
