/**
 * 这是比较页的离线确定性叙述器：将结构化结论、共同场景、赢家特征、可比价格和显著风险组合为指定语言的一段短文。
 * 输入为目录快照、`CompareResult` 与受支持 locale，输出普通字符串；不修改数据、不调用模型或网络，也没有缓存和重试副作用。
 * 价格只在至少两个条目含明确货币符号时参与，百分比或模糊定价被视为不可比，避免从自由文案推导虚假的价格优势。
 * 名称、场景和定价均可能来自卖家/编辑边界，消费组件必须按文本转义；生成的“最稳”措辞不是安全证明、采购授权或实时报价。
 * 少于两个条目返回空串，找不到赢家或指标接近时保留人工判断；该本地路径必须持续可用，以兼容未来模型服务失败或离线场景。
 */
