/**
 * 详情页粘性页眉负责概括代理身份、风险与信任提示，并编排安装/使用、对比、官网和审计入口；实际安装、钱包交易与审计执行不属于本组件。
 * 输入是目录条目、可选主操作回调及动作文案模式，输出为随语言和来源变化的操作栏；回调缺失时仍保持纯展示契约。
 * 本地不持久化状态，对比按钮写入共享选择上下文，主按钮调用宿主回调，链接导航则由浏览器或路由器产生可观察副作用。
 * 目录中的官网 URL 和预填审计字段跨越浏览器信任边界：外链必须隔离 opener，查询参数必须经 `URLSearchParams` 编码，风险/信任标签不能替代权限校验。
 * 各操作需保留可见文字和键盘可达性，达到四项时仅禁止新增而允许移除；缺失外链、非原生代理及可选文案都按现有条件渲染，不能改变旧目录条目的兼容表现。
 */
