/**
 * 主题子系统拥有明暗模式与艺术主题的规范化、持久化和根节点应用规则；它不决定组件内容、权限或任何业务状态。
 * Provider 向后代输出当前主题及显式变更函数，输入来自调用者操作、系统首选项和历史 `localStorage` 值。
 * 状态更新会同步修改 `documentElement` 的类名、`data-theme`、`color-scheme` 并写入本地存储，这是本模块唯一认可的浏览器副作用。
 * 本地存储是可被用户或旧版本篡改的边界：未知值必须归一到默认主题，旧主题仅按映射迁移，服务端/无 DOM 环境必须安全退化。
 * 默认版本键保证设计默认值升级时不会永久沿用过期选择；Context 缺失必须快速失败，主题切换则不得改变页面语义、焦点顺序或可访问名称。
 */
