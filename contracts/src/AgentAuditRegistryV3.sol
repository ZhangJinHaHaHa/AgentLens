...
contract AgentAuditRegistryV3 {
    ...
    struct ReputationRecord {
        ...
        uint32 currentReputationScore;   // 0-10000
        uint64 lastReputationUpdateAt;   // last update timestamp
    }

    // ── MDDRM Constants ─────────────────────────────────────────────
    uint32 public constant BASE_POINTS_PER_AUDIT = 50;       // audit pass base points
    uint32 public constant APPEAL_SUCCESS_BONUS  = 100;      // appeal success bonus
    uint32 public constant APPEAL_FAILURE_PENALTY = 200;     // appeal failure penalty
    uint32 public constant DECAY_RATE_BPS = 100;             // 1% = 100 basis points
    uint64 public constant DECAY_PERIOD = 30 days;           // decay period
    uint32 public constant MAX_REPUTATION_SCORE = 10000;     // upper bound
    ...
    // V3: ReputationUpdated now includes currentScore
    event ReputationUpdated(uint256 indexed tokenId, uint32 currentScore, int32 delta);
    ...
    function recordAuditResult(
        ...
        _recordAuditCore(tokenId, auditScore, status);
        _fillAuditRecord(tokenId, memoryPeakMb, cpuAvgMilli, requestIpCount,
            manifestHash, reportHash, evidenceRoot, attestationHash,
    ...
    function slashBond(
        ...
        // V3: reputation penalty on slash
        ReputationRecord storage rep = _reputations[tokenId];
        if (profile.blacklisted) {
            rep.currentReputationScore = 0;
        } else {
            rep.currentReputationScore = rep.currentReputationScore / 2;
        }
        rep.lastReputationUpdateAt = uint64(block.timestamp);
        emit ReputationUpdated(tokenId, rep.currentReputationScore, rep.reputationDelta);
    ...
    function resolveAppeal(
        ...
        if (outcome == AppealOutcome.Approved) {
            ...
            uint32 newScore = rep.currentReputationScore + APPEAL_SUCCESS_BONUS;
            rep.currentReputationScore = newScore > MAX_REPUTATION_SCORE ? MAX_REPUTATION_SCORE : newScore;
            ...
            rep.failedAppeals += 1;
            rep.reputationDelta -= 1;
            if (rep.currentReputationScore > APPEAL_FAILURE_PENALTY) {
                rep.currentReputationScore -= APPEAL_FAILURE_PENALTY;
            } else {
                rep.currentReputationScore = 0;
            }
        }
    ...
    function _applyTimeDecay(ReputationRecord storage rep) internal {
        ...
        uint64 elapsed = uint64(block.timestamp) - rep.lastReputationUpdateAt;
        if (elapsed == 0) return;
        // decay = score * DECAY_RATE_BPS * elapsed / (DECAY_PERIOD * 10000)
        uint256 decay = uint256(rep.currentReputationScore) * DECAY_RATE_BPS * elapsed
                        / (uint256(DECAY_PERIOD) * 10000);
        if (decay >= rep.currentReputationScore) {
            rep.currentReputationScore = 0;
        } else {
            rep.currentReputationScore -= uint32(decay);
        }
        rep.lastReputationUpdateAt = uint64(block.timestamp);
    }

    function _recordAuditCore(
        uint256 tokenId,
        uint32 auditScore,
        AuditStatus status
    ) internal {
        ...
        emit AuditRecorded(tokenId, record.auditId, status, auditScore, record.reportHash, record.reportCID);

        if (status == AuditStatus.Passed) {
            _accumulateAuditReputation(tokenId, auditScore);
        }
    }

    function _fillAuditRecord(
        uint256 tokenId,
        uint32 memoryPeakMb,
        uint32 cpuAvgMilli,
        uint32 requestIpCount,
        ...
        AuditRecord storage record = _auditRecords[tokenId][_auditRecords[tokenId].length - 1];
    ...
    function _accumulateAuditReputation(uint256 tokenId, uint32 auditScore) internal {
        ReputationRecord storage rep = _reputations[tokenId];
        _applyTimeDecay(rep);
        uint32 contribution = BASE_POINTS_PER_AUDIT * auditScore / 100;
        uint32 newScore = rep.currentReputationScore + contribution;
        rep.currentReputationScore = newScore > MAX_REPUTATION_SCORE ? MAX_REPUTATION_SCORE : newScore;
        rep.lastReputationUpdateAt = uint64(block.timestamp);
        emit ReputationUpdated(tokenId, rep.currentReputationScore, rep.reputationDelta);
    }
