/**
 * 这是审计链监听服务的进程级编排器：持有单实例锁与游标，按轮次冲刷执行/回写/罚没重试队列、轮询新事件、记录任务生命周期并响应停机信号。
 * 它不实现 RPC 解码、沙箱审计、报告存储或交易签名，而是把这些能力从 runtime/state 组合起来；argv/env、链节点、磁盘状态及交易回执均跨越信任边界。
 * 可观察输出包括 JSON 生命周期事件、状态/任务文件、推进后的 nextBlock 以及可选链上回写和罚没；只有完成当前轮处理并持久化游标后才能宣告心跳。
 * 服务锁保证同一 stateDir 只应有一个所有者，三个重试队列按固定顺序串行冲刷；信号只请求在安全边界停止，finally 必须写终态、卸载监听并释放锁。
 * 链交易失败先记录再入持久队列，不能盲目同步重发；事件类型、eventKey 幂等键、游标“下一待扫区块”语义及关闭时的失败传播是运维兼容不变量。
 */
import {
