/**
 * 本文件定义 CDK 与链上注册表之间的稳定数据词汇，包括状态码、配置、代理档案、审计报告和交易回执；它不读取链、不做 ABI 解码，也不持有运行时状态。
 * 输入是其他模块已校验或已解码的值，输出是供命令层和链适配器共享的 TypeScript 合同，其中 bigint 与 0x 前缀哈希用于避免精度和格式歧义。
 * AuditStatus 的数值必须与已部署合约保持向后兼容；未知状态被显式保留为 Unknown，而不是误归类为通过或失败。
 * 接口只表达结构、不提供真实性保证；链数据、CID 和地址仍属于外部信任边界，消费者不得因静态类型而跳过来源校验。
 */
